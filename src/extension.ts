...
function registerTaskProvider(
  ...
  if (vscode.workspace.workspaceFolders) {
    const watcher = vscode.workspace.createFileSystemWatcher("**/hemtt.json");
    watcher.onDidChange(_e => invalidateScriptCaches());
    watcher.onDidDelete(_e => invalidateScriptCaches());
    watcher.onDidCreate(_e => invalidateScriptCaches());
    context.subscriptions.push(watcher);

    const workspaceWatcher = vscode.workspace.onDidChangeWorkspaceFolders(_e =>
      invalidateScriptCaches()
    );
    context.subscriptions.push(workspaceWatcher);

    const provider: vscode.TaskProvider = new HemttTaskProvider();
    const disposable = vscode.workspace.registerTaskProvider("hemtt", provider);
